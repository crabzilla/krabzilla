...
The approach is to use functions everywhere within your domain implementation. For each top level domain model
(Aggregate, ProcessManager/Saga, etc), you need to implement a set of functions. For example, for an Aggregate,
you will implement:
...
[.lead]
*Notice* : By "top level domain model" I mean Aggregate, ExternalSystem, Listener and ProcessManager/Saga. These
are inspired by http://eventstorming.com/[Eventstorming]
